...
This is my first working React app. It was created by following Egghead's [Build Your First Production Quality React App](https://egghead.io/courses/build-your-first-production-quality-react-app) course.

## My learnings
### React cons?

1. Both `state` and `props` are “fluid” objects which can have new properties added to at any time making it difficult to keep track of all available properties by just looking at the code.

Possible solution: `propTypes` can be used for listing all `props` but the component is not necessarily restricted to only those ones.

2. Passing handlers several layers down the component tree via `props`. In this app we declare `handleToggle` and `handleRemove` in `App.js` and then we pass them to `TodoList` just so we can pass it once again to `TodoItem` where they're actually invoked.

Possible solution: Use event emitters?
